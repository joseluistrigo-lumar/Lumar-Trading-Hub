...
from database import init_db, insert_trade, get_all_trades, get_metrics, delete_trade
import io
...
with tab_entry:
    ...
    if st.button("Save Trade", type="primary", use_container_width=True):
        if entry_price <= 0 or exit_price <= 0:
            ...
            pnl, disc = insert_trade(trade_data)
            pnl_label = f"${pnl:,.2f}"
            if pnl >= 0:
...
with tab_history:
    ...
    if not df.empty:
        ...
        for _, row in filtered.iterrows():
            pnl_val = row["pnl"]
            pnl_color_hex = NEON_GREEN if pnl_val > 0 else CRIMSON_RED
